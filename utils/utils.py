...
def preprocess_specific(filename):
    df = pd.read_excel(filename) # load xls file into pandas dataframe
    ...
    df.to_csv(filename+"_preprocessed.csv",index=False) # save dataframe in csv format
    return df
